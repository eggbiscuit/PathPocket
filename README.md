...
- **身份认证** — 邮箱注册 + 邮箱验证 + 管理员审批后登录，对接 FastAPI 后端（见 `backend/`，签发 JWT）
...
- **Phase 4**（进行中）— 真后端联调：FastAPI 认证后端已落地（邮箱注册 + 管理员审批，见 `backend/` 与 `backend/API.md`），客户端默认连真后端；构建时用 `--dart-define=BACKEND_BASE_URL=<地址>` 指向部署。后续：聊天 SSE 代理、Sentry 监控、生产打包
